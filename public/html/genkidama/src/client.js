...
function initPrices() {
	...
	fetch("/api/genkidama/ticker", { "headers": { "Content-Type": "application/json" } })
		.then(res => res.json())
		.then(res => {
			if (res) {
				/*価格を設定するNode*/
...
async function initAmount() {
	for (const pair of PAIRS) {
		fetch("/api/genkidama/positions?symbol=" + pair)
			.then(res => res.json())
			.then(data => {
				if (data) {
					if (data.length > 0) {
